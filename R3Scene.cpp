...
Read(const char *filename, R3Node *node)
{
  ...
  while (fscanf(fp, "%s", cmd) == 1) {
    if (cmd[0] == '#') {
  ...
  else if (!strcmp(cmd, "blueTankSimple") || !strcmp(cmd, "redTankSimple")) {
      // Read data
      int m;
      char meshname[256];
      if (fscanf(fp, "%d%s", &m, meshname) != 2) {
      ...
      R3Line xAxis(0, 0, 0, 1, 0, 0);
      mesh->Rotate(-M_PI/2, xAxis);
      mesh->Scale(.5, .5, .5); //Scale down tank
      if (!strcmp(cmd, "redTankSimple")) {
        redTankSimple = mesh;
      }
      else {
        blueTankSimple = mesh;
    ...
    else if (!strcmp(cmd, "redTank") || !strcmp(cmd, "blueTank")) {
      // Read data
      int m;
      char meshname[256];
      if (fscanf(fp, "%d%s", &m, meshname) != 2) {
      ...
      mesh->Scale(.5, .5, .5); //Scale down tank
      R3Line xAxis(0, 0, 0, 1, 0, 0);
      R3Line yAxis(0, 0, 0, 0, 1, 0);
      mesh->Rotate(-1.57079633, xAxis);
      mesh->Rotate(1.57079633, yAxis);
      
      if (!strcmp(cmd, "redTank")) {
    	  mesh->Translate(-13.5, 0, -13.5); 
      }
      else
    	  mesh->Translate(-13.5, 0, 13.5); //Changed to put on floor
      // Create shape
      R3Shape *shape = new R3Shape();
      shape->type = R3_MESH_SHAPE;
      shape->box = NULL;
      ...
      // Create shape node
      Tank *tank = new Tank();
      //tank->transformation = R3identity_matrix;
      tank->material = material;
      tank->towards = R3Vector(1, 0, 0);
      tank->right = R3Vector(0, 0, 1);
      tank->up = R3Vector(0, 1, 0);
      tank->shape = shape;
      tank->velocity = R3Vector(0, 0, 0);
      tank->shotSpeed = 1.1;
			tank->turn_left = true;
			tank->turn_right = true;
      tank->bulletDelay = 0.0;
      tank->hasFlag = false;
      tank->rotated = 0.0;
			tank->score = 0;
      tank->corners.push_back(R3Point(tank->shape->mesh->bbox.XMin(), tank->shape->mesh->bbox.YMax(), tank->shape->mesh->bbox.ZMin()));
      tank->corners.push_back(R3Point(tank->shape->mesh->bbox.XMin(), tank->shape->mesh->bbox.YMax(), tank->shape->mesh->bbox.ZMax()));
      tank->corners.push_back(R3Point(tank->shape->mesh->bbox.XMax() - .3, tank->shape->mesh->bbox.YMax(), tank->shape->mesh->bbox.ZMin() + .1));
      tank->corners.push_back(R3Point(tank->shape->mesh->bbox.XMax() - .3, tank->shape->mesh->bbox.YMax(), tank->shape->mesh->bbox.ZMax() - .1));
      tank->corners.push_back(R3Point(tank->shape->mesh->bbox.XMax(), tank->shape->mesh->bbox.YMax(), (tank->shape->mesh->bbox.ZMax() + tank->shape->mesh->bbox.ZMin())/2.0));
      tank->corners.push_back(R3Point(tank->shape->mesh->bbox.XMin(), tank->shape->mesh->bbox.YMax(), (tank->shape->mesh->bbox.ZMax() + tank->shape->mesh->bbox.ZMin())/2.0));
      tank->health = 100;
      if (!strcmp(cmd, "redTank")) {
		  // Insert node
		  redTank = *tank;
      ...
      mesh->Scale(.004, .004, .004); //Scale down flag
      R3Line xAxis(0, 0, 0, 1, 0, 0);
      // R3Line yAxis(0, 0, 0, 0, 1, 0);
      mesh->Rotate(-M_PI, xAxis);
      // mesh->Rotate(1.57079633, yAxis);
      mesh->Translate(14, .5, -10); //Changed to put on floor
      // Create shape
      R3Shape *shape = new R3Shape();
      shape->type = R3_MESH_SHAPE;
      shape->box = NULL;
      ...
      flag->towards = R3Vector(1, 0, 0);
      flag->right = R3Vector(0, 0, 1);
      flag->up = R3Vector(0, 1, 0);
      flag->shape = shape;

      // Insert node
      greenFlag = *flag;
    }
    else if(!strcmp(cmd, "bullet")) {
        // Read data
        int m;
        char meshname[256];
        if (fscanf(fp, "%d%s", &m, meshname) != 2) {
      ...
      camera.xz = R3Vector(0, 0, 1);
    }
    else if (!strcmp(cmd, "cameraTwo")) {
          // Read data
          double px, py, pz, dx, dy, dz, ux, uy, uz, xfov, neardist, fardist;
          if (fscanf(fp, "%lf%lf%lf%lf%lf%lf%lf%lf%lf%lf%lf%lf", &px, &py, &pz, &dx, &dy, &dz, &ux, &uy, &uz, &xfov, &neardist, &fardist) != 12) {
            fprintf(stderr, "Unable to read camera at command %d in file %s\n", command_number, filename);
          ...
          // Assign camera
          camera2.eye = R3Point(px, py, pz);
          camera2.towards = R3Vector(dx, dy, dz);
          camera2.towards.Normalize();
          camera2.up = R3Vector(ux, uy, uz);
          camera2.up.Normalize();
          camera2.right = camera2.towards % camera2.up;
          camera2.right.Normalize();
          camera2.up = camera2.right % camera2.towards;
          camera2.up.Normalize();
          camera2.xfov = xfov;
          camera2.yfov = xfov;
          camera2.neardist = neardist;
          camera2.fardist = fardist;
          camera2.xz = R3Vector(0, 0, 1);
        }
